...
import { zoneSummary } from '@/features/storage/mockData';
...
export function ZoneSummaryDonut() {
  const total = zoneSummary.reduce((s, d) => s + d.count, 0);
  const pieData = zoneSummary.map((d) => ({ name: d.label, value: d.count, color: d.color }));
  ...
  return (
    <div className="relative w-full" style={{ height: 140 }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
        ...
        <span className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider mt-0.5">Total Zones</span>
      </div>
    </div>
  );
}
