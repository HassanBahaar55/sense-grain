...
/** Seasonal temperature bonus: peaks in May–Jun for South Asia (+3°C) */
function seasonal(date: Date): number {
  const m = date.getMonth(); // 0–11, peak at 4 (May)
...
export function getWarehouseReadings(date: Date): WarehouseReading[] {
  ...
  const active: WarehouseReading[] = WH_BASES.map(b => {
    ...
    const status: WHStatus =
      temp >= 32 || humidity >= 74 || moisture >= 15 ? 'high' :
      temp >= 29 || humidity >= 65 || moisture >= 13 ? 'medium' : 'good';

    const risk: RiskLevel =
      status === 'high' ? 'high' :
      status === 'medium' ? 'medium' : 'low';

    const trendRaw = r(`${key}-tr-${b.id}`);
    const trend: TrendDir =
      trendRaw < 0.25 ? 'down' :
      trendRaw < 0.50 ? 'stable' :
      trendRaw < 0.75 ? 'slight-up' : 'up';
    ...
    return {
      ...
      status, risk, trend,
      temp, humidity, moisture, co2, aqi,
      capacity: b.cap,
      used,
...
export function getDashboardData(date: Date): DashboardData {
  ...
  return {
    warehouses,
    alerts: limited,
    goodCount, watchCount, criticalCount, offlineCount,
...
export function getAlertsData(date: Date): AlertsData {
  ...
  const alertsByType = [
    ...
    { label: 'Capacity',    count: typeMap.capacity,    color: '#f97316' },
...
export interface StorageData {
  warehouses: WarehouseReading[];
  totals: {
    totalWarehouses: number; totalZones: number; totalCapacity: number; totalUsed: number;
    usedPct: number; avgTemp: number; avgHumidity: number; activeCount: number; highRiskUnits: number;
  };
...
export function getStorageData(date: Date): StorageData {
  ...
  return {
    ...
    totals: {
      ...
      avgTemp, avgHumidity, activeCount: active.length,
...
export interface WHPerformancePoint {
  warehouse: string; Efficiency: number; Stability: number; Utilization: number;
}

export interface AnalyticsKPI {
  label: string; value: number; unit: string; delta: string; trend: TrendDir;
...
export function getAnalyticsData(date: Date): AnalyticsData {
  ...
  function tempStability() {
    const bad = active.filter(w => (w.temp ?? 0) >= 29).length;
    return Math.max(50, Math.round(100 - bad * 8 + rr(`${key}-tst`, -3, 3)));
  }
  function humStability() {
    const bad = active.filter(w => (w.humidity ?? 0) >= 65).length;
    return Math.max(50, Math.round(100 - bad * 6 + rr(`${key}-hst`, -3, 3)));
  }

  const ts = tempStability(), hs = humStability();
  ...
  const envTrendData: EnvTrendPoint[] = Array.from({ length: 14 }, (_, i) => {
    ...
    const dKey = dk(d);
    // Convert readings to 0-100 stability scores
    return {
      day: dayLabel(d),
  ...
  return {
    ...
    sensorSummary: {
      ...
      total: onlineCount + offlineCount + warningCount,
    },
  };
}
...
export interface ParamForecastCard {
  ...
  current: number; forecast: number; rangeMin: number; rangeMax: number;
  trend: TrendDir; confidence: number; sparkData: number[];
  colorKey: 'amber' | 'blue' | 'green' | 'purple' | 'teal' | 'indigo';
}

export interface ForecastPoint { day: string; [key: string]: number | string }

export interface WHPredRow {
  id: string; name: string; overallRisk: RiskLevel;
  spoilage30d: number;
  tempForecast: number; humForecast: number; moistForecast: number;
  co2Forecast: number; aqiForecast: number; capForecast: number;
  trend: TrendDir; confidence: number;
}
...
export function getPredictionsData(date: Date): PredictionsData {
  ...
  const paramCards: ParamForecastCard[] = [
    ...
    { key: 'capacity', label: 'Storage Capacity',  unit: '%',   current: Math.round(active.reduce((s, w) => s + w.usedPct, 0) / active.length), forecast: 0, rangeMin: 0, rangeMax: 0, trend: 'up', confidence: ri(`${key}-capc`, 88, 96), sparkData: [], colorKey: 'indigo' },
  ];
  const capCard = paramCards[5];
  capCard.forecast  = Math.min(95, Math.round(capCard.current + rr(`${key}-cap-f`, 1, 3)));
  capCard.rangeMin  = capCard.current - 2;
  capCard.rangeMax  = capCard.current + 5;
  capCard.sparkData = mkSpark(capCard.current - 1, 7, 0.1, 0.4, 'capsp');

  // 7-day forecast lines (% of threshold)
...
export interface ReportItem {
  id: string; type: ReportType; title: string; warehouse: string; period: string;
  generatedAt: string; dateGenerated: string; generatedBy: string;
  size: string; downloads: number; status: 'ready' | 'processing' | 'scheduled';
}
...
export function getReportsData(date: Date): ReportsData {
  ...
  const recentReports: ReportItem[] = Array.from({ length: 8 }, (_, i) => {
    ...
    return {
      ...
      generatedAt: when, dateGenerated: dateStr, generatedBy: genBy,
  ...
  for (const r of recentReports) typeCountMap[r.type]++;
  ...
  const reportTrendData = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      day: dayLabel(d),
      Generated:  ri(`${dKey}-rgen`, 2, 12),
      Downloaded: ri(`${dKey}-rdld`, 1, 8),
    };
  });
...
// ─── Parameter trends chart data ──────────────────────────────────────────────

export interface ParamTrendPoint {
  time: string;
  temp: number; humidity: number; moisture: number; co2: number;
}
